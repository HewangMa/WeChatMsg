...
class Waken:
    def __init__(self, date_str) -> None:
        ...
        self.time_range = (
            date_str, self.next_day.strftime("%Y-%m-%d")+HOUR_TIME)
    ...
    def waken_one_day(self):
        msg_in_one_day = self.__parse_chat_msg(self.time_range)
        ...
        for p_idx in range(part_num):
            ...
            if end_idx < msg_cnt-1:
                end_idx += self.bias
            logger.info(
                f"Trying with start = {start_idx}, end = {end_idx}, msg_cnt = {msg_cnt}")
    ...
    def __exact_and_write(self, waken_res_all):
        paras = waken_res_all.split("{{split}}")
        to_write = ['\n', self.header] + \
            [self.beauty + '\n' + p for p in paras if len(p) > 2]
        file_path = os.path.join(WAKEN_RES_ROOT, self.file_name)
